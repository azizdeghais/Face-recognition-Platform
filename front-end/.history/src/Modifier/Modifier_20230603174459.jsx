   ...
   const blob = new Blob([new Uint8Array(byteArrays)], { type: 'image/jpeg' });
    ...
    const handleFormSubmit = async (event) => {
      ...
      const handleStartTime = (event) => {
        const selectedDateTime = DateTime.fromJSDate(event).setZone('Europe/Berlin', { keepLocalTime: true });
        if (selectedDateTime.isValid) {
          const formattedDateTime = selectedDateTime.toISO();
          setStartTime(selectedDateTime.toJSDate());
      ...
      const handleEndTime = (event) => {
        ...
        if (selectedDateTime.isValid) {
          const formattedDateTime = selectedDateTime.toISO();
          setEndTime(selectedDateTime.toJSDate());
